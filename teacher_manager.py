# -*- coding: utf-8 -*-
"""
教师管理模块 - 提供教师信息的增、删、改、查（数据存储于 MySQL）
"""
...
def add_teacher(current_user):
    ...
    execute_query(
        ...
        (tid, name, gender, age, title, department, phone,
         email, research, remark, now, now),
    )
    print(f"  [成功] 教师 '{name}' 已添加，教师编号：{tid}")
    pause()
...
def search_teacher(current_user):
    ...
    if choice == "1":
        ...
        results = execute_query(
            "SELECT * FROM teachers WHERE teacher_id = %s", (tid,), fetch=True
        )
    elif choice == "2":
...
def update_teacher(current_user):
    ...
    title_input = input(
        f"  职称 [{teacher['title']}] ({'/'.join(sorted(TITLES))}，留空不变): "
    ).strip()
    ...
    if research_input:
        fields["research"] = research_input
    ...
    if fields:
        ...
        execute_query(f"UPDATE teachers SET {set_clause} WHERE teacher_id = %s", values)  # noqa: S608
        print("  [成功] 教师信息已更新。")
    else:
        print("  未作任何修改。")
    pause()
